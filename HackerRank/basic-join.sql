...
each non-evil wand of high power and age. Write a query to print the id, age, coins_needed, 
...
WITH wands_table AS(
    SELECT w.id,
           ...
           w.coins_needed,
           w.power,
           /*Gives a rank number to each row and then sorts by coins in ascending order*/
           RANK() OVER 
                (PARTITION BY wp.age, w.power ORDER BY w.coins_needed ASC)
                AS rank_number
    FROM wands AS w
...
FROM wands_table
/*We only want the cheapest wand from each of the windows*/
WHERE rank_number = 1
ORDER BY power DESC,
         age DESC;
...
then sort the result by hacker_id. I
f more than one student created the same number of challenges and the count is less than the maximum number of challenges created, 
...
WHERE total_challenges = (SELECT MAX(total_challenges) FROM hacker_challenges)
                                    OR
      total_challenges IN 
